import { ChevronLeft, ChevronRight, Edit2, Plus, Search, Trash2, Users } from 'lucide-react';
...
interface SchoolClassGroupsManagementProps {
...
export function SchoolClassGroupsManagement({ sessionId }: SchoolClassGroupsManagementProps) {
    ...
    useEffect(() => {
        if (department && sessionId) fetchGroups();
        else setGroups([]);
    ...
    const fetchGroups = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.getClassGroups({ department, session_id: sessionId });
            const data = response.success ? response.data : null;
            const list = Array.isArray(data) ? data : (data?.class_groups || []);
            setGroups(list);
        } catch (error: any) {
    ...
    const filteredGroups = searchQuery.trim()
        ? groups.filter(
            (g) =>
                String(g.level).toLowerCase().includes(searchQuery.toLowerCase()) ||
                String(g.name).toLowerCase().includes(searchQuery.toLowerCase())
        )
        : groups;
    const paged = filteredGroups.slice((page - 1) * itemsPerPage, page * itemsPerPage);
    const maxPage = Math.max(1, Math.ceil(filteredGroups.length / itemsPerPage));
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="space-y-1">
                        ...
                        <select
                            ...
                            className="w-full min-w-[200px] px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#ffb71b]"
                        >
                            ...
                            <div className="relative flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <input
                                        id="class-search"
                                        type="text"
                                        placeholder="Level or group..."
                                        value={searchQuery}
                                        onChange={(e) => setSearchQuery(e.target.value)}
                                        className="w-full pl-8 pr-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#ffb71b]"
                                    />
                                </div>
                                <Button type="button" variant="outline" size="sm" className="h-9 shrink-0" onClick={() => document.getElementById('class-search')?.focus()}>
                ...
                <>
                    <div className="overflow-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {paged.map((g) => (
                                    <tr key={g.group_id ?? g.id} className="border-b hover:bg-slate-50">
                                        ...
                                        <td className="p-3 text-right">
                                            ...
                                            <Button size="sm" variant="destructive" onClick={() => handleDelete(g.group_id ?? g.id)} title="Delete">
