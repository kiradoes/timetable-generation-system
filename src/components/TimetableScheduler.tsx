...
export function TimetableScheduler({
  ...
  const [selectedSession, setSelectedSession] = useState(activeSessionName || '');
  ...
  const levels = Array.from(new Set(classGroups.map((group) => String(group.level))))
    .sort()
    .reverse();
  const groups = classGroups
    .filter((group) => String(group.level) === selectedLevel)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [coursesRes, lecturersRes, classGroupsRes] = await Promise.all([
          api.getCourses({ ...params, category: 'Computing' }) as any,
        ...
        if (classGroupsRes.success) {
          setClassGroups(classGroupsRes.data?.class_groups || []);
        }
      } catch (error) {
  ...
  const checkConflicts = (entry: typeof newEntry): string[] => {
    ...
    const lecturerConflict = scheduleEntries.find(
      (e) =>
        e.lecturerId === entry.lecturerId &&
        e.day === entry.day &&
        e.timeSlot === entry.timeSlot
    );
    if (lecturerConflict) {
      const lecturerName = lecturers.find((l) => String(l.lecturer_id) === entry.lecturerId)?.name || 'This lecturer';
      conflicts.push(`${lecturerName} has another class at this time`);
    }

    const classGroupConflict = scheduleEntries.find(
      (e) =>
        e.classGroup === `${selectedLevel}${selectedGroup}` &&
        e.day === entry.day &&
        e.timeSlot === entry.timeSlot
    );
    if (classGroupConflict) {
      conflicts.push(`Class group ${selectedLevel}${selectedGroup} is already scheduled at this time`);
    }
  ...
  const handleAddCourse = () => {
    if (!newEntry.courseId || !newEntry.lecturerId || !newEntry.venue) {
      toast.error('Please select a course, lecturer, and venue');
      return;
    }
    ...
    const selectedCourse = courses.find((course) => String(course.course_id) === newEntry.courseId);
    const selectedLecturer = lecturers.find((lecturer) => String(lecturer.lecturer_id) === newEntry.lecturerId);
          ...
          <CardContent className="space-y-4 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select value={newEntry.courseId} onValueChange={(value: string) => setNewEntry({ ...newEntry, courseId: value })}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                <Label htmlFor="lecturerId">Select Lecturer *</Label>
                <Select value={newEntry.lecturerId} onValueChange={(value: string) => setNewEntry({ ...newEntry, lecturerId: value })}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select a lecturer" />
                  </SelectTrigger>
                  <SelectContent>
                    {lecturers.map((lecturer) => (
                      <SelectItem key={lecturer.lecturer_id} value={String(lecturer.lecturer_id)}>
                        {lecturer.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select value={newEntry.venue} onValueChange={(value: string) => setNewEntry({ ...newEntry, venue: value })}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select value={newEntry.day} onValueChange={(value: string) => setNewEntry({ ...newEntry, day: value })}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value: string) => setNewEntry({ ...newEntry, timeSlot: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
