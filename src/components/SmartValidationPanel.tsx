...
interface ValidationIssue {
  id: string;
  type: 'naming-error' | 'capacity' | 'repetition-needed';
  severity: 'error' | 'warning' | 'info';
  message: string;
  suggestion?: string;
  affectedClass: ScheduledClass;
}
  ...
  const detectValidationIssues = (): ValidationIssue[] => {
    ...
    scheduledClasses.forEach((cls: ScheduledClass) => {
      ...
      if (requiredCourses.length > 0) {
        const exactMatch = requiredCourses.find((c: Course) => c.code.toUpperCase() === cleanCode);
        
        if (!exactMatch) {
          // Find similar courses (potential typos)
          const similar = requiredCourses.find((c: Course) => {
