...
export function TimetableViewer({ userRole, department, activeSessionId }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      let sid = activeSessionId ?? null;
      if (!sid) {
        try {
          const r = await api.getCurrentSession();
          if (r?.success && r?.data) sid = r.data.session_id ?? r.data.id ?? null;
        } catch (_) {}
      }
      if (cancelled) return;
      setSessionId(sid);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.getSpecialEvents({ session_id: sessionId });
        if (cancelled || !res?.success) return;
        setSpecialEvents(res.data || []);
      } catch (_) {
        if (!cancelled) setSpecialEvents([]);
      }
    })();
    return () => { cancelled = true; };
  }, [sessionId]);
